...
function CastCard({ person }: { person: CastPerson }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 min-w-[80px] max-w-[80px]">
...
export function MediaDetail({ jellyfinId, title, type, itemType }: MediaDetailProps) {
  ...
  // Fetch Jellyfin details (only on initial load / jellyfinId change)
  ...
  // Fetch episodes for a specific season (called when season changes)
  const fetchEpisodes = useCallback(async (seasonId: string) => {
    ...
    try {
      ...
      const data = await res.json()
      // Only update the episodes, keep other details
  ...
  // Combine overview from both sources (prefer Jellyfin, fallback to TMDB)
  ...
  // Combine cast (prefer Jellyfin people, fallback to TMDB cast)
  ...
  return (
    <div className="mt-4 space-y-6">
      {/* ─── Synopsis / Overview (only for Jellyfin items; non-Jellyfin items show description in VideoPlayer) ── */}
      {jellyfinId && (overview || tagline) && (
        <div className="bg-muted/30 rounded-xl p-4">
          {tagline && (
            <p className="text-sm italic text-muted-foreground mb-2">&ldquo;{tagline}&rdquo;</p>
          )}
          {overview && (
            <>
              <p
